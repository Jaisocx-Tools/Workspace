...
export class Tooltip extends EventEmitter implements TooltipInterface {
  ...
  _hide( node: HTMLElement|null ): undefined {
    ...
    node.classList.remove (
      TooltipConstants.CssClassNames.TOOLTIP_SHOWN
    );


    //@ts-ignore
    node.classList.add (
      TooltipConstants.CssClassNames.TOOLTIP_HIDDEN
    );
  }
  ...
  addClickCurrentTooltipCloseEventHandler(): Tooltip {
    ...
    document.getElementsByTagName("HTML")[0].addEventListener (
      ...
      (evt: Event) => {
        ...
        //@ts-ignore
        let holderTooltip: HTMLElement|null = evt.target.closest(`.${TooltipConstants.CssClassNames.TOOLTIP_MAIN}`);
  ...
  showTooltip (
    ...
    if ( toShow === TooltipConstants.ShowModes.SHOW ) {
      ...
      if (
        (
          ( this.tooltipHideBehaviour === TooltipConstants.TooltipHideBehaviour.HIDE_AFTER_TIMEOUT__AND__WHEN_CLICK__ANYWHERE ) ||
  ...
  hideAllTooltips(): undefined {
    this.hideTooltipsByBehaviours (
      [ ...( TooltipConstants.TooltipHideBehaviour as any ) ],
      null
    );
  }
  ...
  hideTooltipsByBehaviours (
    ...
    let hideBehaviourTooltipConstantsRelevantToTimeout: any[] = [
      TooltipConstants.TooltipHideBehaviour.HIDE_AFTER_TIMEOUT__AND__WHEN_CLICK__ANYWHERE,
      TooltipConstants.TooltipHideBehaviour.HIDE_AFTER_TIMEOUT__AND__WHEN_CLICK__OTHER_THAN_EVENT_TARGET,
      TooltipConstants.TooltipHideBehaviour.HIDE_AFTER_TIMEOUT__AND__WHEN_CLICK__EVENT_TARGET
    ];
  ...
  setStandardCssClassAndGetDimensions (
    ...
    const cssClassnameHiddenShownPrefix: any = TooltipConstants.CssClassNames.TOOLTIP_CLASSES_HIDDEN_SHOWN_PREFIX;
    ...
    const cssClassWithTransitionSet: any = classList.contains (
      TooltipConstants.CssClassNames.TOOLTIP_CLASSES_HIDDEN_SHOWN_WITH_TRANSITION
    );
    ...
    classList.add (
      TooltipConstants.CssClassNames.TOOLTIP_SHOWN_SIMPLE
    );
