export class CssHtml {
  ...
  remToPx( 
    remValue: string, 
    inRemBasePxValue: number ): string|false {
    ...
    let remValNumber: number = +( remValue.substring( 0, substringEndPos ) );
  ...
  resolveCssValueIfVariable( 
    ...
    let variableName: string = cssValue.substring( 
      substringStartPos, 
      substringEndPos );
    let cssVariableValue: string = this.getVariableValue( element, variableName );
