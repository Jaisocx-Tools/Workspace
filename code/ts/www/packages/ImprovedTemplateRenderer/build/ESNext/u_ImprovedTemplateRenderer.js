...
export class ImprovedTemplateRenderer extends EventEmitter {
    ...
    prepareTaggedGroupsInfos() {
        ...
        firstLoopNextIterationStart: for (let i = 0; i < this.dataConf.length; i++) {
            ...
            let infos = this.prepareTaggedNestedSubcallGetInfosOneGroup(i, nestedTag);
            if (infos === null) {
                // skip to the next dataConf elem
                continue firstLoopNextIterationStart;
            }
    ...
    matchedTagName(tagToMatch, templateConfTags) {
        let taggedMatched = 1;
        if (!templateConfTags || templateConfTags.length === 0) {
            ...
            let foundMatchedTag = templateConfTags
                .find((tag) => (tag === tagToMatch));
            if (!foundMatchedTag) {
                taggedMatched = 0;
            }
        }
    ...
    prepareTaggedNestedSubcallGetInfosOneGroup(holderTemplateIndex, tagName) {
        let loopIndexToStart = holderTemplateIndex + 1;
        let helpingObjectNestedTagged = {
            holderTemplateName: "",
            tag: tagName,
        ...
        let taggedMatched = 0;
        let taggedMatchedNumber = 0;
        let taggedMatchedLastIndex = 0;
        let innerTaggedGroupMatchedNumber = 0;
        let innerTemplateNestedTagname = tagName;
        let innerTemplatesMaxLevel = 0; // this points, the inner level is the first.
        let innerLevelsInfoArray = []; // the first inner level tag name applied.
        loopLevelOne: for (let i = loopIndexToStart; i < this.dataConf.length; i++) {
            let templateName = this.dataConf[i].templateName;
            let templateConfElem = this.templatesConf[templateName];
            let tagOrArrayOfTags = templateConfElem.tag;
    ...
    calculateResultArrayRecursiveNumber() {
        let resultArrayRecursiveElemsNumber = 0;
        let templatesConfKeys = Object.keys(this.templatesConf);
        let objectToIterateOver = this.dataConf;
        let templatesAmount = objectToIterateOver.length;
        ...
        let dataRule = {};
        let templateRuleMultiple = 0;
        let templateRuleMultipleNested = 0;
        let helpingObjectNestedTagged = {
            holderTemplateName: "",
            tag: "",
            holderIndex: 0,
        ...
        for (templatesIndex = 0; templatesIndex < templatesAmount; templatesIndex++) {
            ...
            if ((templateRuleMultipleNested === 1)) {
                ...
                if ((tagName.length > 0)) {
                    ...
                    for (let j0 = j; j0 < templatesAmount; j0++) {
                        ...
                        let tagOrTagsArray = taggedNestedDataRule.tag;
    ...
    nestedTemplatesRender(nestedPropName, key, value, recordset) {
        ...
        const amountNestedPropsMatched = this.nestedTemplatesDataPointersArrays[nestedPropName].length;
        let templateDataArrayIndex = 0;
        let nestedTemplatesIndex = 0;
        let nestedTemplatesSize = amountNestedPropsMatched;
        let nestedTemplateData = {};
        let nestedTemplateName = "";
        let nestedTemplate = "";
        let renderedTemplate = null;
        let nestedTemplateConf = {};
        let nestedPropName2 = "";
        for (nestedTemplatesIndex = 0; nestedTemplatesIndex < nestedTemplatesSize; nestedTemplatesIndex++) {
            ...
            if (nestedPropName2) {
                ...
                renderedTemplate = this
                    .nestedTemplatesRender(nestedPropName2, nestedTemplateConf, value, recordset);
            }
            else {
                renderedTemplate = this
    ...
    calculateNestedPropsTemplatesDataPointersArray(nestedPropName) {
...
function recordRenderSubcall(propName, propValue, templatesObject, templateData) {
...
function whenNoCharThenFillCharsCodeRange(inputArgCharCode) {
    ...
    for (charsCodeRangesKey of charsCodeRangesKeys) {
        ...
        if ((charCodeLowToCompare < inputArgCharCode) &&
...
//# sourceMappingURL=u_ImprovedTemplateRenderer.js.map